...
unsafe fn reverse_stone(mode: i8 , prev : i8 , xy: &[usize;2] , x: i8, y: i8) -> bool{ //mode 0 = check 1 = reverse ,xy는 현재위치 xy[1] = x && xy[0]= y, x,y는 진행방향
...
unsafe fn chk_chacksu(mode: i8 ,turn_state : i8 , ij : &[usize;2]) -> bool{ //mode 0 = check 1 = reverse 
    ...
    for i in 0..8{
        ...
        if STONE_STATE[tmpy][tmpx] == enemy_stone && reverse_stone(mode,enemy_stone,&[tmpy,tmpx],x[i],y[i]){
            flag = true;
        }
    }
